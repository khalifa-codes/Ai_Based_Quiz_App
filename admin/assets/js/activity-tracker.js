...
(function() {
    ...
    function getApiPath() {
        ...
        // Determine if we're in admin root or subdirectory
        if (currentPath.includes('/admin/')) {
            const pathAfterAdmin = currentPath.split('/admin/')[1];
            if (pathAfterAdmin && pathAfterAdmin.trim() !== '') {
                // Count directory levels (e.g., "plans/plan_list.php" = 1 level)
                const parts = pathAfterAdmin.split('/').filter(p => p && p !== '');
                // Remove the PHP file name, count only directories
                const directories = parts.filter(p => !p.includes('.php'));
                if (directories.length > 0) {
                    return '../'.repeat(directories.length) + 'api/admin/activity.php';
                }
            }
            // We're in admin root (e.g., /admin/dashboard.php)
            return 'api/admin/activity.php';
        }
        // Fallback - should not happen in admin module
        return 'api/admin/activity.php';
    }
    ...
    function handleSessionExpired() {
        ...
        if (currentPath.includes('/admin/')) {
            ...
            if (pathAfterAdmin && pathAfterAdmin.trim() !== '') {
                ...
                const directories = parts.filter(p => !p.includes('.php'));
                if (directories.length > 0) {
                    loginPath = '../'.repeat(directories.length) + 'login.php';
                }
            }
        }
